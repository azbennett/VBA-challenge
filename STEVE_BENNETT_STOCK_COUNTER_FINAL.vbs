Sub stockcounter()
    ...
    Dim rng As Range                    ' search area size
    ...
    Dim bestincname As String           'names for above 3
    Dim bestdecname As String
    Dim bestvolname As String
    
    
    ReDim currentsheet(1 To ActiveWorkbook.Sheets.Count) 'sets the array size to match the qty of sheets
    ...
    For sheetcycle = 1 To ActiveWorkbook.Sheets.Count 'runs the primary outer loop for how many sheets we have
        ...
        howmanyrows = Worksheets(currentsheet(sheetcycle)).UsedRange.Rows.Count ' figures out how many vertical rows it has
        howmanyrows = howmanyrows - 1 ' this adjusts for removing the header so the loop does not run 1 too many times
    
        Set rng = Worksheets(currentsheet(sheetcycle)).Range("A2:G" & howmanyrows) ' sets the range to match the above row counter
        ...
        For rowCount = 1 To howmanyrows 'this loops for how many rows DOWN we have
            ...
            If StrComp(tickername, namecheck, 1) <> 0 Then ' checking if the current stock name matches the next one, if not we output the results
                ...
                'MsgBox "I just verified that " & tickername & " and " & namecheck & " do not match."
        ...
        'END OF SHEET - BEGIN PURGE OF DATA SO NOTHING CARRIES OVER INTO THE NEXT
        ...
        'PURGE COMPLETE - START NEXT SHEET CYCLE
